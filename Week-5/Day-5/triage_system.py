...
def search_kb_tool(query: str) -> str:
    ...
    if "corrupt" in query.lower():
        raise FileNotFoundError("knowledge_base.json not found or inaccessible.")
...
def route_ticket(state: TicketState):
    ...
    elif cat == "Billing": return "billing"
    elif cat == "Invalid" or cat == "Refusal": return "failure"
    else: return "general"
...
builder.add_node("human_review", lambda x: x) # No-op node just to act as checkpoint
...
triage_app = builder.compile(checkpointer=memory, interrupt_before=["human_review"])
